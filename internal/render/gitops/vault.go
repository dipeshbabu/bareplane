...
for _, condition in ipairs(status.conditions or {}) do
  if condition.type == "Ready" then
    ...
    if condition.status == "True" then
      ...
      if type(observed) == "number" and observed >= 1 and string.sub(status.syncedResourceVersion or "", 1, string.len(generation)) == generation and status.refreshTime ~= nil then
