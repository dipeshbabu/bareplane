...
func RenderProxmox(cfg config.Config, publicKey string) ([]byte, error) {
	...
	for _, machine := range desired.Machines {
		...
		resources[address] = map[string]any{
			...
			"cpu": []any{
				map[string]any{"cores": machine.CPU},
			},
			...
			"memory": []any{
				map[string]any{"dedicated": machine.MemoryGB * 1024},
			},
			"name": tfString(machine.Name),
			"network_device": []any{
				map[string]any{"bridge": tfString(settings.Bridge)},
			},
			"node_name": tfString(machine.Target),
			"on_boot":   true,
			"operating_system": []any{
				map[string]any{"type": "l26"},
			},
	...
	document := map[string]any{
		...
		"provider": map[string]any{
			"proxmox": []any{
				map[string]any{"endpoint": tfString(cfg.Spec.Provider.Endpoint)},
			},
		},
